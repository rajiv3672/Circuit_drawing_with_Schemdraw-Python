...
with schemdraw.Drawing() as d:
    ...
    d += (L9 := elm.Line().at(op.vd).up().length(0.8).dot())
    d += (L10 := elm.Line().at(op.vs).down().length(0.8).dot())
    d += (L11 := elm.Line().at(L9.end).right().length(1))
    d += (L12 := elm.Line().at(L10.end).right().length(1))
    d += (S2 := elm.Battery().at(L11.end).right().dot().label('$+V_{CC}$ \n $+15V$', 'bot'))
    d += (S3 := elm.Battery().at(L12.end).right().reverse().dot().label('$-V_{EE}$ \n $-15V$', 'bot'))
    d += (L13 := elm.Line().endpoints(S2.end, S3.end))
    d += (G3 := elm.Ground().at(S3.end))